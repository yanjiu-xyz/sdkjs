...
(function(){
    ...
    function CPushButtonField(sName, nPage, aRect)
    {
        ...
        CTextField.prototype.SetAlign.call(this, ALIGN_TYPE.center);
    ...
    CPushButtonField.prototype.AddImage = function(oImgData) {
        ...
        if (private_getViewer().IsOpenFormsInProgress == false) {
            let oDoc            = this.GetDocument();
            let oActionsQueue   = oDoc.GetActionsQueue();
        ...
        if (this.GetDrawing()) {
            ...
            if ((this._buttonPosition == position["iconTextH"] || this._buttonPosition == position["textIconH"])  && this.content.GetElementsCount() == 1) {
                ...
                if (this._buttonPosition == position["iconTextH"])
                    CTextField.prototype.SetAlign.call(this, ALIGN_TYPE.right);
                else if (this._buttonPosition == position["textIconH"])
                    CTextField.prototype.SetAlign.call(this, ALIGN_TYPE.left);
    ...
    CPushButtonField.prototype.Recalculate = function() {
        ...
        if (this.GetButtonFitBound() == false) {
    ...
    CPushButtonField.prototype.GetButtonFitBound = function() {
    ...
    CBaseCheckBoxField.prototype.SetValue = function(sValue) {
        ...
        if (private_getViewer().IsOpenFormsInProgress && this.GetParent() == null)
    ...
    CBaseCheckBoxField.prototype.SetChecked = function(bChecked) {
        ...
        if (bChecked) {
            !private_getViewer().isOnUndoRedo && AscCommon.History.Add(new CChangesPDFFormValue(this, this.GetValue(), this._exportValue));
            this._checked = true;
        }
        else {
            !private_getViewer().isOnUndoRedo && AscCommon.History.Add(new CChangesPDFFormValue(this, this.GetValue(), "Off"));
            this._checked = false;
        }
    };
    ...
    CTextField.prototype.Draw = function() {
        ...
        let oContentToDraw = this.GetTrigger(FORMS_TRIGGERS_TYPES.Format) && this.IsNeedDrawHighlight() ? this.contentFormat : this.content;
    ...
    CTextField.prototype.EnterText = function(aChars)
    {
        ...
        let { startPos, endPos } = private_CalcDocPos(oPara, nSelStart, nSelEnd);
    ...
    CTextField.prototype.GetAlign = function() {
        ...
        switch (nParaAlign) {
            case align_Left:
                nFormAlignType = ALIGN_TYPE.left;
                break;
            case align_Center:
                nFormAlignType = ALIGN_TYPE.center;
                break;
            case align_Right:
                nFormAlignType = ALIGN_TYPE.right;
                break;
            default:
                nFormAlignType = ALIGN_TYPE.left;
                break;
    ...
    CTextField.prototype.Remove = function(nDirection, bWord) {
        ...
        let { startPos, endPos } = private_CalcDocPos(this.content.GetElement(0), nSelStart, nSelEnd);
        if (nSelStart == nSelEnd) {
            this.content.SetContentPosition(startPos, 0, 0);
            this.content.RecalculateCurPos();
        }
	 ...
	 * @typeofeditors ["PDF"]
	 */
    function private_CalcDocPos(oPara, nStart, nEnd)
	{
    ...
    function private_evalStr(str) {
        ...
        return func.bind(oDoc.GetDocumentApi(), ...Array(aArgsNamesToDelete.length - 1), oApiConsole, ...aArgsPdfApi);
    };
	...
	window["AscPDF"].private_getGlobalCoordsByPageCoords  = private_getGlobalCoordsByPageCoords;
