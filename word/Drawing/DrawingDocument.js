...
function CDrawingDocument()
{
	...
	this.DrawTocPreview = function(canvas, index, pixW)
	{
		let prop = null;
		if (0 === index)
		{
		...
		// content
		let oLogicDocument   = this.m_oWordControl.m_oLogicDocument;
		let oStyles          = oLogicDocument.GetStyles();
		let oHeader          = new CHeaderFooter(oLogicDocument.HdrFtr, oLogicDocument, this, AscCommon.hdrftr_Header);
		let oDocumentContent = oHeader.GetContent();

		let arrLevels         = [];
		let arrStylesToDelete = [];

		let wPxOffset = AscBrowser.convertToRetinaValue(8, true);
		let wMmOffset = wPxOffset * g_dKoef_pix_to_mm / AscCommon.AscBrowser.retinaPixelRatio;
	...
	this.GetTOC_Buttons = function(idDiv1, idDiv2, styleWidth)
	{
		...
		for (var i = 0; i < 2; i++)
		{
			this.DrawTocPreview((i === 0) ? canvas1 : canvas2, i, pixW);
		}
